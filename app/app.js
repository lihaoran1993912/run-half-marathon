...
import {
  ...
  weekProgress, estimateFinishDate,
...
function render() {
  ...
  if (complete) {
    ...
    card.innerHTML = `
      ...
      <div class="next-head">接下来 · 第 ${s.seq} 次（计划里标注为 ${s.day}）</div>
      <div class="next-detail">${s.detail}</div>
      <div class="wc"><span>热身</span><span>${s.warmup}</span></div>
      <div class="wc"><span>放松</span><span>${s.cooldown}</span></div>
      <div class="focus"><b>教练的话：</b>${s.focus}</div>
      <div class="check-row">
        <input type="date" id="ciDate" value="${todayStr()}" max="${todayStr()}">
...
function renderWeeks(state) {
  ...
  for (let wk = 1; wk <= TOTAL_WEEKS; wk++) {
    const inWeek = PLAN.filter((s) => s.week === wk);
    const rec = inWeek[0].recovery;
    html += `<div class="wk"><h3>第 ${wk} 周 · 第 ${inWeek[0].month} 月 ${inWeek[0].phase}${rec ? ' <span class="r">减量</span>' : ''}</h3>`;
    for (const s of inWeek) {
      const isDone = s.seq <= done;
      const isNow = s.seq === done + 1;
      html += `<div class="sess ${isDone ? 'done' : ''} ${isNow ? 'now' : ''}">
        <span class="mark">${isDone ? '✓' : isNow ? '→' : '·'}</span>
        <span class="d">${s.day}</span>
        <span class="t">${s.detail}</span>
      </div>`;
    }
    html += '</div>';
  }
